...
def customers():
    ...
    # working on customers query to the database based on user role and filters
    ...
    customers = sorted(
        customers,
        key=lambda x: status_order.get(x.get("status"), 12)
    )
...
def download_customers():
    ...
    for c in customers:
        data.append({
            ...
            "Application Date": c.get("date_applied").strftime("%Y-%m-%d") if isinstance(c.get("date_applied"), datetime.datetime) else c.get("date_applied"),
            ...
            "Survey Date": c.get("survey_date").strftime("%Y-%m-%d") if isinstance(c.get("survey_date"), datetime.datetime) else c.get("survey_date"),
    ...
    if es_report_start_date_str and es_report_end_date_str:
        ...
        for c in customers:
            ...
            data.append({
                ...
                "Connection Fee": c.get("connection_fee"),
                "Initial Amount Paid for Connection": c.get("amount_paid"),
                "Date of Initial Connection Payment": c.get("date_paid").strftime("%Y-%m-%d") if c.get("date_paid") else None,
...
def download_bp_reports():
    ...
    for c in customers:
        data.append({
            ...
            "Connection Fee": c.get("connection_fee"),
            "Date of Initial Connection Payment": c.get("date_paid").strftime("%Y-%m-%d") if c.get("date_paid") else None,
            "Connection date": c.get("connection_date").strftime("%Y-%m-%d") if c.get("connection_date") else None,
